...
- 加强了待机和工作中动画。Codex 工作状态会播放几轮第 7 行后回到第 0 行循环，所以第 0 行现在改成后台思考循环，避免长时间任务里豆包傻站不动。
...
- `idle`：后台思考循环。Codex 工作状态回落到 idle 后，豆包仍会保持思考手势和轻微动势。
...
`tools/enhance_idle_work_rows.py` 用来加强待机和工作状态：第 0 行改成后台思考循环，第 7 行保留思考手势并增加轻微上下动势。
